...
wave_clean <- wave_clean %>%
  mutate(septic_shock = if_else(grepl("Septic Shock", complications), 
         ...
         mi = ifelse(grepl('Myocardial Infarction', complications), 
                                1, 
                                ifelse(complications == '', NA, 0)), 
         hf = ifelse(grepl("Heart failure", complications), 
                                1, 
                                ifelse(complications == '', NA, 0)), 
...
obj1_tb1 <- wave_clean %>%
  select(sex, race_eth, age_cat, obesity, cancer_factor, cva_comorbidity,  gi_condition, gi_symptoms, comp_factor, intubation1, death, wave, septic_shock, arrhythmias, mi, hf, renal_replacement1, vte1) %>%
