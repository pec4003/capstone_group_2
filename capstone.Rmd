...
# descriptive stats
obj1_tb1 <- wave_clean %>%
  select(sex, race_eth, age_cat, obesity, cancer_factor, dm_factor, gi_condition, gi_symptoms) %>%
...
# descriptive stats
obj3_tb1 <- wave_clean %>%
  select(sex, race_eth, age_cat, obesity, cancer_factor, dm_factor, gi_condition, life_threat_comp, intubation1, death) %>%
  tbl_summary(by = gi_condition,  missing = 'no', 
              label = list(age_cat ~ "Age", 
                         race_eth ~ "Race",
                         intubation1 ~ "Intubation")) %>%
