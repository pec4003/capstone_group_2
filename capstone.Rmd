...
wave$race_eth <- factor(wave$race_eth, levels=c("4", "2", "3", "1", "5"))
levels(wave$race_eth) <- c("White", "Black", "Asian", "Hispanic", "Other")
...
wave$cva_comorbidity <- factor(wave$cva_comorbidity, levels=c(0, 1), labels=c("No", "Yes"))
label(wave$cva_comorbidity) <- "Cardio Comorbidity"
...
```{r}
fit_obj1 <- glm(gi_symptoms ~ gi_condition + sex + race_eth + age_cat + obesity + cancer_factor + cva_comorbidity, wave_clean, family = binomial)
summary(fit_obj1)
...
wave_clean$outcome <- factor(wave_clean$outcome, levels=c(0, 1), labels=c("No", "Yes"))


fit_obj2 <- glm(outcome ~ gi_symptoms + sex + race_eth + age_cat + obesity + cancer_factor + cva_comorbidity, wave_clean, family = binomial)
summary(fit_obj2)
```
